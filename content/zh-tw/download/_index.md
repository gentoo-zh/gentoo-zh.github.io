...
- **ZFS 根 + 原生加密 + ZBM 引導（進階）** — 安裝器分割槽頁檔案系統可選 **ZFS**；勾選「加密」即啟用 **ZFS 原生加密**（aes-256-gcm、口令解鎖），並由 **ZFSBootMenu** 原生引導（GRUB 讀不了帶新特性 / 原生加密的 ZFS 池，故 ZFS 根改用 ZBM）。預設檔案系統為 xfs，btrfs / ext4 / ZFS 均可在分割槽頁選。
