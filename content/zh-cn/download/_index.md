...
- **ZFS 根 + 原生加密 + ZBM 引导（进阶）** — 安装器分区页文件系统可选 **ZFS**；勾选「加密」即启用 **ZFS 原生加密**（aes-256-gcm、口令解锁），并由 **ZFSBootMenu** 原生引导（GRUB 读不了带新特性 / 原生加密的 ZFS 池，故 ZFS 根改用 ZBM）。默认文件系统为 xfs，btrfs / ext4 / ZFS 均可在分区页选。
